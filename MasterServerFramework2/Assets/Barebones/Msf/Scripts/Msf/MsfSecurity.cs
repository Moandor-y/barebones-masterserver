...
namespace Barebones.MasterServer
{
    ...
    {
        public delegate void PermissionLevelCallback(
            int? permissionLevel, string error);
        ...
        public void RequestPermissionLevel(string key,
                                           ...
                                           IClientSocket connection)
        {
            connection.SendMessage(
                (short) MsfOpCodes.RequestPermissionLevel,
                key,
                (status, response) =>
                {
        ...
        /// sends it to master, which returns encrypted AES key. After
        /// decrypting AES key, callback is invoked with the value. You can
        /// then use the AES key to encrypt data
        /// </summary>
        public async Task<string> GetAesKey(IClientSocket connection)
        {
            EncryptionData data;
            ...
            if (data == null)
            {
                ...
                // Generate keys
                data.ClientsPublicKey = await Task.Run(
                    () => data.ClientsCsp.ExportParameters(false));
            }

            if (data.ClientAesKey != null)
            {
                // We already have an aes generated for this connection
                return data.ClientAesKey;
            }
            ...
            // Send the request
            connection.SendMessage(
                (short) MsfOpCodes.AesKeyRequest,
                publicKey,
                (status, response) =>
                {
                    if (data.ClientAesKey != null)
                    {
                        ...
                        result.TrySetResult(data.ClientAesKey);
                        return;
                    }
                    ...
                    var decrypted =
                        data.ClientsCsp.Decrypt(response.AsBytes(), false);
        ...
        {
            var disconnected = _encryptionData
                              .Keys.Where(c => !c.IsConnected).ToList();
        ...
        public async Task<byte[]> EncryptAES(byte[] rawData,
                                             string sharedSecret)
        {
            return await Task.Run(() =>
            {
                using (var aesAlg = new RijndaelManaged())
                {
                    ...
                    Rfc2898DeriveBytes key =
                        new Rfc2898DeriveBytes(sharedSecret, _salt);
                                             ...
                                             CryptoStreamMode.Write))
                        {
                            using (var writer =
                                new EndianBinaryWriter(
                                    EndianBitConverter.Big, csEncrypt))
                            {
        ...
        public async Task<byte[]> DecryptAES(byte[] encryptedData,
                                             string sharedSecret)
        {
            return await Task.Run(() =>
            {
        ...
        public async Task<string> EncryptStringAES(
            string plainText, string sharedSecret)
        {
            return await Task.Run(() =>
            {
                string outStr;
        ...
        /// decryption.</param>
        public async Task<string> DecryptStringAES(
            string cipherText, string sharedSecret)
        {
            return await Task.Run(() =>
            {
        ...
        public async Task<string> CreateHash(string password)
        {
            return await Task.Run(() =>
            {
                ...
                var hash = PBKDF2(password, salt, PBKDF2_ITERATIONS,
                                  HASH_BYTE_SIZE);
