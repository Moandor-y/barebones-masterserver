...
namespace Barebones.MasterServer
{
    ...
    {
        public delegate void SpawnRequestHandler(
            SpawnRequestPacket packet, IIncommingMessage message);
        ...
        public DefaultSpawnerConfig Settings
        {
            get { return DefaultSpawnerSettings; }
        }
        ...
        public static BmLogger Logger =
            Msf.Create.Logger(typeof(SpawnerController).Name, LogLevel.Warn);

        private static object _processLock = new object();

        private static Dictionary<int, Process> _processes =
                                 ...
                                 SpawnerOptions options)
        {
            ...
            // Add handlers
            connection.SetHandler((short) MsfOpCodes.SpawnRequest,
                                  HandleSpawnRequest);
            connection.SetHandler((short) MsfOpCodes.KillSpawnedProcess,
        ...
        public void NotifyProcessStarted(int spawnId, int processId,
                                         string cmdArgs)
        {
            Msf.Server.Spawners.NotifyProcessStarted(
                spawnId, processId, cmdArgs, Connection);
        }
        ...
        {
            Msf.Server.Spawners.UpdateProcessesCount(
                SpawnerId, count, Connection);
        }

        private void HandleSpawnRequest(SpawnRequestPacket packet,
        ...
        private static void HandleSpawnRequest(IIncommingMessage message)
        {
            ...
            if (controller == null)
            {
                if (message.IsExpectingResponse)
                    message.Respond("Couldn't find a spawn controller",
        ...
        {
            Logger.Debug(
                "Default kill request handler started handling a request to kill a process");
        ...
        public void DefaultSpawnRequestHandler(SpawnRequestPacket packet,
                                               IIncommingMessage message)
        {
            Logger.Debug(
                "Default spawn handler started handling a request to spawn process");

            var controller =
                Msf.Server.Spawners.GetController(packet.SpawnerId);

            if (controller == null)
            {
                message.Respond(
                    "Failed to spawn a process. Spawner controller not found",
            ...
            var masterIp =
                string.IsNullOrEmpty(controller.DefaultSpawnerSettings.MasterIp)
            ...
            // Get the scene name
            var sceneNameArgument =
                packet.Properties.ContainsKey(MsfDictKeys.SceneName)
                    ? string.Format("{0} {1} ", Msf.Args.Names.LoadScene,
                                    packet.Properties[MsfDictKeys.SceneName])
                    : "";
            ...
            var spawnInBatchmode =
                controller.DefaultSpawnerSettings.SpawnInBatchmode
            ...
            var startProcessInfo = new ProcessStartInfo(path)
            {
                ...
                Arguments =
                    ...
                    (controller.DefaultSpawnerSettings.AddWebGlFlag
                        ? Msf.Args.Names.WebGl + " "
                        : "") +
                    sceneNameArgument +
                    $"{Msf.Args.Names.MasterIp} {masterIp} " +
                    $"{Msf.Args.Names.MasterPort} {masterPort} " +
                    $"{Msf.Args.Names.SpawnId} {packet.SpawnId} " +
                    $"{Msf.Args.Names.AssignedPort} {port} " +
                    $"{Msf.Args.Names.MachineIp} {machineIp} " +
                    (Msf.Args.DestroyUi ? Msf.Args.Names.DestroyUi + " " : "") +
                    $"{Msf.Args.Names.SpawnCode} \"{packet.SpawnCode}\" " +
                    packet.CustomArgs
            };

            Logger.Debug("Starting process with args: " +
                         startProcessInfo.Arguments);
            ...
            try
            {
                new Thread(() =>
                {
                    try
                    {
                        ...
                        {
                            Logger.Debug("Process started. Spawn Id: " +
                                         packet.SpawnId + ", pid: " +
                                         process.Id);
                            processStarted = true;
                            ...
                            BTimer.ExecuteOnMainThread(() =>
                            {
                                ...
                                controller.NotifyProcessStarted(
                                    packet.SpawnId, processId,
                                    startProcessInfo.Arguments);
                            });
                    ...
                    finally
                    {
                        ...
                        BTimer.ExecuteOnMainThread(() =>
                        {
                            ...
                            Logger.Debug(
                                "Notifying about killed process with spawn id: " +
                                packet.SpawnerId);
